...
OSS.prototype.getUrlAuthorization = function(bucket, object, expireSecond) {
  var expires = Math.floor(Date.now() / 1000) + expireSecond;
  var str = 'GET\n\n\n' + expires + '\n/' + bucket + '/' + object;
  var basicString = crypto.createHmac('sha1', this.accessKeySecret).update(str);
  return 'OSSAccessKeyId=' + this.accessKeyId + '&Expires=' + expires + '&Signature=' + basicString.digest('base64');
};
 ...
 */
[
  'createBucket', 'listBucket', 'deleteBucket', 'getBucketAcl', 'setBucketAcl',
  'putObject', 'copyObject', 'deleteObject', 'getObject', 'headObject', 'listObject'
].forEach(function(method) {
  OSS.prototype[method] = thenify(OSS.prototype[method]);
});
